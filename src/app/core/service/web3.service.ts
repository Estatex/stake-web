...
export class Web3Service {
        ...
        this.providerOptions = {
            ...
            'custom-walletlink': {
                ...
                options: {
                    ...
                    chainId: this.configToken.ETH_NETWORK.Web3Modal.network,
                },
                package: WalletLink,
    ...
    setWeb3WalletData(provider:any, address:any):void{
        ...
        this.isWalletConnected = true;
        localStorage.setItem('accountAddress', this.walletAddress)
    }
    ...
    getValidValue(valu:any,decimal:number = 2,tokenDecimal?:number) {
        if (valu > 0) {
            let usrBal;
            if(tokenDecimal){
                usrBal = valu / Math.pow(10,tokenDecimal);
            } else {
                usrBal = this.web3js.utils.fromWei(valu, 'gwei')
            }
            return Number(usrBal).toFixed(decimal)
        }
        return valu
    }
    ...
    checkUser(){
        this.pledgeService.check_user({walletAddress:this.walletAddress}).subscribe({
          next: (data) => {
            if(data.type === true){
              if(data.data.email) {
                ...
                this.router.navigate(['veriff-verification']);
              }
            } else {
